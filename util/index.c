...
// Index loading lifecycle
//  index_t owns the loaded star tree, quad table, code k-d tree, FITS handle and metadata strings
// resources are released by index_unload(), index_close(), and index_free()
...
// reloads index components if missing: starkd: reference star k-d tree,quads, codekd
