...
solver_codekd_packet_query_verification_ready(
    ...
    *more_work = FALSE;
    ...
    /*
     * Retain the broad topology-ready prefix, but execute and capture only the
     * next logical retirement slice. Exact sweep pages depend on native query
     * results, so bounding this stage avoids processing the complete topology
     * prefix before its earliest candidates can retire. The owner rearm path
     * restores verify_plan_end from verify_topology_end for the next slice.
     */
    plan_end = packet->verify_plan_first + MIN(
        packet->verify_plan_end - packet->verify_plan_first,
        (size_t)SOLVER_VERIFY_RETIRE_HORIZON);
    ...
    for (candidate_index = packet->verify_plan_first;
        ...
        if (!query) {
            ...
            if (packet->verify_query_bytes >
                packet->verify_query_budget - query_bytes) {
                verify_destroy_index_query(query);
...
/*
 * A compute worker consumes only fully captured native query results, creates
 * a bounded contiguous set of index-free contexts, and scores that packet in
 * place. The packet claim is exclusive, verify_field_t is immutable, and no
 * solver, reducer, index mapping, or callback state is accessed. Scientific
 * effects remain deferred to ordered owner retirement.
 */
index_shard_helper_task_status_t
solver_codekd_packet_prepare_and_score_verification_ready(
    ...
    if (!input || !packet || !packet->candidate_records ||
        ...
        packet->verify_plan_end <= packet->verify_plan_first ||
        packet->verify_plan_end > packet->candidate_window_count ||
        packet->verify_topology_end < packet->verify_plan_end ||
        packet->verify_topology_end >
            packet->candidate_window_count ||
    ...
    /*
     * Topology delivery may cover a broad physical prefix. Query execution,
     * exact sweep capture, preparation, and scoring remain bounded to the
     * current retirement slice; verify_topology_end preserves the wider
     * topology horizon for later owner-ordered slices.
     */
    original_plan_end = packet->verify_plan_end;
    if (original_plan_end - packet->verify_plan_first >
            SOLVER_VERIFY_RETIRE_HORIZON) {
        original_plan_end = packet->verify_plan_first +
            SOLVER_VERIFY_RETIRE_HORIZON;
    ...
    for (candidate_index = verify_index;
        ...
        if (query_bytes == SIZE_MAX ||
            ...
            verify_prepare_captured_hit_from_query(
                &record->verify_query,
