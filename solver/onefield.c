...
void onefield_run(onefield_t *bp)
{
    ...
    if (bl_size(bp->verify_wcs_list))
    {
        ...
        for (w = 0; w < bl_size(bp->verify_wcs_list); w++)
        {
            ...
            quadlo = bp->quad_size_fraction_lo * MIN(wcs->wcstan.imagew, wcs->wcstan.imageh) * pixscale;
            quadhi = bp->quad_size_fraction_hi * MAX(wcs->wcstan.imagew, wcs->wcstan.imageh) * pixscale;
