...
#define INDEX_SHARD_READY_BEFORE_OUTER_BURST_MAX 2U
...
typedef struct index_shard_worker_context {
  ...
  /*
   * After completing one outer index, this worker may execute one bounded
   * burst of foreign COMPUTE_READY packets before its next outer claim.
   * The reducer grants the burst; only successful foreign claims consume it.
   */
  size_t ready_before_outer_budget;
