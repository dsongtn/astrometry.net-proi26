 ...
 */
// search field-star combinations for current index
...
void solver_run(solver_t *solver)
{
    ...
    num_indexes = pl_size(solver->indexes); // active indexes for this solver run
    {
        ...
        for (i = 0; i < num_indexes; i++)
        {
            ...
            index_t *index = pl_get(solver->indexes, i); // get active index by solver-local order
        ...
        for (newpoint = solver->startobj; newpoint < numxy; newpoint++)
        {
            ...
            for (i = 0; i < num_indexes; i++)
            {
                index_t *index = pl_get(solver->indexes, i);
                int dimquads;
                set_index(solver, index); // update current index state
                ...
                for (field[A] = 0; field[A] < newpoint; field[A]++)
                {
                    ...
                    add_stars(pq, field, C, dimquads - 2, 0, newpoint, dimquads, solver, tol2); // search field-star combinations for current index
                    if (solver->quit_now)
                        goto quitnow;
                }
            }
            ...
            if ((solver->maxquads && (solver->numtries >= solver->maxquads)) || (solver->maxmatches && (solver->nummatches >= solver->maxmatches)) || solver->quit_now)
                break;
        }
...
// verifies a quad/code candidate, optionally refines the WCS
// records accepted matches through the callback, and updates solver-local
// sp must be worker-loca
// sp->record_match_callback must not mutate shared onefield_t state in thread design
                             ...
                             anbool fake_match)
{
    ...
    verify_hit(sp->index->starkd, sp->index->cutnside,
               ...
               sp->logratio_stoplooking,
               sp->distance_from_quad_bonus, fake_match); // verify candidate against current index and field
    mo->nverified = sp->num_verified++;                   // assign and increment solver-local verify counter

    if (mo->logodds >= sp->best_logodds)
    {
        sp->best_logodds = mo->logodds; // update solver-local best score
    ...
    solved = (!sp->record_match_callback ||
              sp->record_match_callback(mo, sp->userdata)); // record accepted match
    ...
    if (!sp->have_best_match || (mo->logodds > sp->best_match.logodds))
    {
        ...
        memcpy(&sp->best_match, mo, sizeof(MatchObj)); // shallow copy, pointer fields are shared
        sp->have_best_match = TRUE;
        sp->best_index = sp->index; // remember index that produced best match
    }
    else
    {
        verify_free_matchobj(mo);
