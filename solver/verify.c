                ...
                for (; j < i; j++)
                    if (theta[j] < 0)
                    {
                        ...
                        debug2("  adjusting distractor %i: %g change in logodds\n",
                               j, (logd_at(distractors, muj, v->NR, logbg) - logd_at(distractors, muj + 1, v->NR, logbg)));
                    }
                    else
                        muj++;
...
Verification core:
This function evaluates a candidate WCS by projecting reference stars
into image coordinates and comparing them against the preprocessed field.
Most temporary arrays are local to this call and are freed before return.
Parallelization note:
The function is structurally close to reentrant if skdt and vf are
read-only. However, thread safety still depends on startree_search_for(),
kdtree traversal, and any FITS-backed data access used underneath.
*/
                ...
                anbool do_gamma, anbool fake_match)
{
    ...
    if (K >= logaccept)
    {
        ...
        // MatchObj data export
        // accepted verification, dynamically allocated verification arrays are
        // transferred into MatchObj. The caller and callback later manage ownership
        //  transfer path must remain unchanged during serial shard emulation
        mo->theta = etheta;
        mo->matchodds = eodds;
        mo->refxyz = refxyz;
        refxyz = NULL;
